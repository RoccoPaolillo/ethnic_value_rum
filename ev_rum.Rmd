...
# for asym condition ethnic size (majority 80%, minority 20%)
...
df_asm_pic2 <- df %>% gather(key = group_asmsd, value = Segra_asmsd, cls_et_sq_bl_sd,cls_et_cl_bl_sd,cls_et_sq_or_sd,cls_et_cl_or_sd,
                               cls_vl_sq_bl_sd,cls_vl_cl_bl_sd,cls_vl_sq_or_sd,cls_vl_cl_or_sd)%>%
  mutate(`outcome` = fct_relevel(fct_collapse(group_asmsd,
                                              "Ethnicsd" = c("cls_et_sq_bl_sd","cls_et_cl_bl_sd","cls_et_sq_or_sd","cls_et_cl_or_sd"),
                                              "Valuesd" = c( "cls_vl_sq_bl_sd","cls_vl_cl_bl_sd","cls_vl_sq_or_sd","cls_vl_cl_or_sd")), "Ethnicsd"),
         `type` = fct_relevel(fct_collapse(group_asmsd,
                                           "Conservative Native" = c("cls_et_sq_bl_sd","cls_vl_sq_bl_sd"),
                                           "Liberal Native" = c("cls_et_cl_bl_sd","cls_vl_cl_bl_sd"),
                                           "Conservative Migrant" = c("cls_et_sq_or_sd","cls_vl_sq_or_sd"),
                                           "Liberal Migrant" = c("cls_et_cl_or_sd","cls_vl_cl_or_sd")),"Conservative Native" ))

# coefficient of variation

df_asm_pic <- cbind(df_asm_pic, df_asm_pic2[setdiff(names(df_asm_pic2), names(df_asm_pic))])
df_asm_pic <- df_asm_pic[c(setdiff(names(df_asm_pic), "group_asm"), "group_asm")]
df_asm_pic <- df_asm_pic[c(setdiff(names(df_asm_pic), "group_asmsd"), "group_asmsd")]
df_asm_pic <- df_asm_pic[c(setdiff(names(df_asm_pic), "Segra_asm"), "Segra_asm")]
df_asm_pic <- df_asm_pic[c(setdiff(names(df_asm_pic), "Segra_asmsd"), "Segra_asmsd")] 

df_asm_pic$coeffvar <- df_asm_pic$Segra_asmsd / df_asm_pic$Segra_asm


# asymmetric condition based on value distribution (50% ethnic, 80%/20% value distribution)
...
df_asm_bsl2 <- df %>% gather(key = group_asmsd, value = Segra_asmsd,
                            et_sq_bl_sd,et_cl_bl_sd,et_sq_or_sd,et_cl_or_sd,
                              vl_sq_bl_sd,vl_cl_bl_sd,vl_sq_or_sd,vl_cl_or_sd,
                            den_cl_bl_sd,den_sq_bl_sd,den_cl_or_sd,den_sq_or_sd
                            )%>%
  mutate(`outcome` = fct_relevel(fct_collapse(group_asmsd,
"Ethnicsd" = c("et_sq_bl_sd","et_cl_bl_sd","et_sq_or_sd","et_cl_or_sd"),
                                              "Valuesd" = c( "vl_sq_bl_sd","vl_cl_bl_sd","vl_sq_or_sd","vl_cl_or_sd"),
                                              "Densitysd" = c(
"den_sq_or_sd","den_cl_or_sd","den_sq_bl_sd","den_cl_bl_sd")
), "Ethnicsd"),
         `type` = fct_relevel(fct_collapse(group_asmsd,
                         "Conservative Native" = c("et_sq_bl_sd","vl_sq_bl_sd","den_sq_bl_sd"),
                         "Liberal Native" = c("et_cl_bl_sd","vl_cl_bl_sd","den_cl_bl_sd"),
                         "Conservative Migrant" = c("et_sq_or_sd","vl_sq_or_sd","den_sq_or_sd"),
                         "Liberal Migrant" = c("et_cl_or_sd","vl_cl_or_sd","den_cl_or_sd")),
                              "Conservative Native" )) 
  
# coefficient of variation

df_asm_bsl <- cbind(df_asm_bsl, df_asm_bsl2[setdiff(names(df_asm_bsl2), names(df_asm_bsl))])
df_asm_bsl <- df_asm_bsl[c(setdiff(names(df_asm_bsl), "group_asm"), "group_asm")]
df_asm_bsl <- df_asm_bsl[c(setdiff(names(df_asm_bsl), "group_asmsd"), "group_asmsd")]
df_asm_bsl <- df_asm_bsl[c(setdiff(names(df_asm_bsl), "Segra_asm"), "Segra_asm")]
df_asm_bsl <- df_asm_bsl[c(setdiff(names(df_asm_bsl), "Segra_asmsd"), "Segra_asmsd")] 

df_asm_bsl$coeffvar <- df_asm_bsl$Segra_asmsd / df_asm_bsl$Segra_asm
...
# libval <- df_bsl_pic %>% filter(ID=="sens_lib" & orientation_bsl == "Conservative") %>%
#   ggplot(aes(x=con_eth,y=Segra_bsl,color=outcome_bsl,linetype=factor(lib_val, levels = c("20","0")))) +
#   geom_point(size=1.5, shape = "square") + geom_line() +
#    scale_linetype_manual(name = "ß value liberal", values= c("0" = "dotted","20" = "solid")) +
#   scale_color_manual(name = "Outcome\nConservatives",
#                      values = c("Ethnic" = "purple","Value" = "darkgreen", "Density" = "gray"),
#                      labels = c("Ethnic" = "Ethnic segregation", "Value" = "Value segregation","Density" = "Density")) +
#   ylab("Outcome conservative") +
#   xlab("ß ethnic conservative") +
#  # ggtitle("Effect liberal value preferences over conservative agents") +
#   theme_bw()
#       # labels = c("0" = "ß value liberal = 0", "20" = "ß value liberal = 20")
#     
# coneth <- df_bsl_pic %>% filter(ID=="sens_con" & orientation_bsl == "Liberal") %>%
#   ggplot(aes(x=lib_val,y=Segra_bsl,color=outcome_bsl,linetype=factor(con_eth, levels = c("20","0")))) +
#   geom_point(size=1.5, shape = "circle") + geom_line() +
#    scale_linetype_manual(name = "ß ethnic conservative", values= c("0" = "dotted","20" = "solid")) +
#   scale_color_manual(name = "Outcome\nLiberals",
#                      values = c("Ethnic" = "purple","Value" = "darkgreen", "Density" = "gray"),
#                      labels = c("Ethnic" = "Ethnic exposure", "Value" = "Value exposure","Density" = "Density")) +
#   ylab("Outcome liberal") +
#   xlab("ß value liberal") +
# #  ggtitle("Effect conservative ethnic preferences over liberal agents") +
#   theme_bw()
# 
# ggpubr::ggarrange(libval,coneth,nrow = 2)
# ggsave("img_analysis/totsens.jpg",width = 7,height = 7)
...
libv <- df_bsl_pic %>% filter(ID=="sens_lib" & orientation_bsl == "Conservative") %>%
  ...
  scale_color_manual(name = "Outcome",
                     values = c("Ethnic" = "purple","Value" = "darkgreen", "Density" = "gray"),
                     labels = c("Ethnic" = "Ethnic\nsegregation", "Value" = "Value\nsegregation","Density" = "Density")) +
...
ggsave("img_analysis/totsens2.jpg",width = 7,height = 5)
...
```{r asym_value_annex_checkSD, include = TRUE}
...
geom_point(size =2,shape="circle") +
  ...
  theme_bw() + 
 scale_color_manual("% Liberal Native / % Liberal Migrant", values = c("50" = "coral2", "60" = "olivedrab", "70" = "cyan", "80" = "violet"), 
...
pic <- annotate_figure(ggpubr::ggarrange(et,vl, common.legend = T, legend = "bottom",nrow = 2), 
                top = text_grob(sbj, size = 12))
ggsave(pic, file = paste0("img_analysis/",sbj,"_vlsz.jpg"), width = 6, height = 9)
}
...
```{r asym_ethnic_annexSD, include = TRUE}
...
picet <- annotate_figure(ggpubr::ggarrange(et,vl, common.legend = T, legend = "bottom",nrow = 2), 
                top = text_grob(sbj, size = 12))
ggsave(picet, file = paste0("img_analysis/",sbj,"_etsz.jpg"), width = 6, height = 9)
# ggpubr::ggarrange(et,vl, common.legend = T, legend = "bottom")
}
